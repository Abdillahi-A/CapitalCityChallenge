...
def display_high_scores(name, correct_answers, conn):
    c = conn.cursor()
    c.execute("INSERT INTO highScores (username, highscore) VALUES (?,?)", (name, correct_answers))
    conn.commit()

    print('\n')
    print("HighScores:")

    for row in c.execute('SELECT username, highscore FROM highScores ORDER BY highscore desc LIMIT 5'):
...
def main():
    ...
    while True:  
        ...
        display_high_scores(name, correct_answers, conn)
        if not play_again():
            leave_game()
            break
